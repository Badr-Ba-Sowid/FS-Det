...
class PointNetfeat(nn.Module):
    def __init__(self, device, global_feat = True, feature_transform = False):
        ...
        if self.feature_transform:
            self.fstn = STNkd(device= device, k=64)
            self.fstn.to(device)
    def forward(self, x):
        n_pts = x.size()[2]
        trans = self.stn(x)
...
class PointNetCls(nn.Module):
    def __init__(self, k, device, feature_transform=False):
...
class PointNetDenseCls(nn.Module):
    def __init__(self, k = 2, feature_transform=False):
        ...
        self.k = k
        self.feature_transform=feature_transform
        self.feat = PointNetfeat(device=device, global_feat=False, feature_transform=feature_transform)
